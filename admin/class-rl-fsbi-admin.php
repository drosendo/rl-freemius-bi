...
class RL_FSBI_Admin {
	...
	public function display_plugin_dashboard( $forced_plugin_id = 0 ) {
		...
		$GLOBALS['rl_fsbi_forced_plugin_id'] = (int) $forced_plugin_id;
	...
	public function handle_sync_ajax() {
		...
		if ( empty( $plugins ) ) {
			wp_send_json_error( esc_html__( 'Sync failed: no plugins returned by Freemius for current credentials.', 'rl-freemius-bi' ) );
		}
		...
		if ( empty( $plugin_ids ) ) {
			wp_send_json_error( esc_html__( 'Sync skipped: no selected plugins matched current discovery catalog.', 'rl-freemius-bi' ) );
		}
	...
	private function sync_freemius_data() {
		...
		$plugins = $api->retrieve_plugins();
		if ( empty( $plugins ) ) {
			$this->settings->update_option( 'rl_fsbi_last_sync_status', esc_html__( 'Sync failed: no plugins returned by Freemius for current credentials.', 'rl-freemius-bi' ) );
			$this->settings->update_option( 'rl_fsbi_last_sync_at_utc', gmdate( 'Y-m-d H:i:s' ) );
			return false;
		}
		...
		foreach ( $plugins as $plugin ) {
			...
			if ( ! empty( $selected_plugin_ids ) && ! in_array( $plugin_id, $selected_plugin_ids, true ) ) {
				continue;
			}
		...
		while ( true ) {
			...
			foreach ( $payments as $payment ) {
				...
				$data = array(
					...
					'transaction_date' => sanitize_text_field( $payment['created'] ?? current_time( 'mysql' ) ),
	...
	public function handle_get_dashboard_data_ajax() {
		...
		$previous_end = gmdate( 'Y-m-d', strtotime( $start_date . ' -1 day' ) );
		...
		$annual_start = gmdate( 'Y-m-01', strtotime( '-11 months', strtotime( $end_date ) ) );
	...
	private function build_sales_activity_series( $payments, $subscriptions, $start_date, $end_date, $commission_rate = 0.0, $report_currency = 'EUR' ) {
		...
		foreach ( $subscriptions as $subscription ) {
			$trial_date = ! empty( $subscription->trial_ends ) ? substr( (string) $subscription->trial_ends, 0, 10 ) : null;
